<link href="<?php echo base_url(); ?>/assets/css/mainpage.css" rel="stylesheet">

<style>
.wf-section {
    padding-top: 96px;
    padding-bottom: 96px;
}

.col-md-5 {
    padding-bottom: 60px;
}

.flip-card-front h4 {
    font-weight: 600;
}

.row.d-flex.h-677.mt-144 {
    padding-bottom: 100px !important;
}
</style>

<section class="top p-0 mt-5">
    <div class="hero-section ehite ">
        <div>
            <div class="container">
                <div class="row d-flex h-677 ">
                    <div class="p1-hero-row-left z-1">
                        <h1 class="hero-title"> Join The Team which will help Shape the Future of
                            <span class="orange">Laboratory </span>
                        </h1>
                        <p class="p mt-32 _w-582">In case you don’t find a relevant position, feel free to write to us
                            at <a href="mailto:  info@labexperts.ae"> info@labexperts.ae</a>
                        </p>
                        <div class="hero-btn-wrapper">
                           
                            <!--- <a  class="link-block w-inline-block">
                  <div class="text-block">Find your solution</div>
                  <img src="https://assets.website-files.com/602cf2b08109ccbc93d7f9ed/602e761f1ca5476d49049024_Vector%20(Stroke).svg" loading="lazy" alt="" class="img _2" style="
                      transform: translate3d(0px, 0px, 0px) scale3d(1, 1, 1)
                        rotateX(0deg) rotateY(0deg) rotateZ(0deg)
                        skew(0deg, 0deg);
                      transform-style: preserve-3d;
                    " />
                </a> -->
                        </div>
                    </div>
                    <div class="p1-hero-row-right">
                        <div class="hero-img-wrapper">

                            
                        </div>
                    </div>
                </div>
                <hr>
            </div>
        </div>
    </div>

    <div class="hero-section">
        <div>
            <div class="container ">
                <h2 class="aside-title ">
                    Latest Jobs
                </h2>
                <style>
                .card-career {
                    padding: 20px;
                    border: 2px solid #c9c3c3;

                }
                </style>
                <div class="row container" style="padding-top:70px;">
                    <div class="col-md-4 mb-3"><span style="height:5px;background:#f26522;display: block;"></span>
                        <div class="cmp-teaser__content card-career" style="min-height:280px;">
                            <div class="row " style="justify-content: space-between;">

                                <div class="col-md-4">
                                    <div class="cmp-teaser__pretitle cmp-teaser-region" style="font-weight: 600;">DUBAI
                                    </div>
                                </div>


                                <div class="col-md-2 text-center">
                                    <i class="bi bi-star"></i>
                                    <span class="save-job-label" style="font-weight: 600;">
                                        SAVE
                                    </span>
                                </div>

                            </div>
                            <a href="<?php echo base_url('Receptionist '); ?>">
                                <h4 class="mt-3" style="font-weight: 900;"> Female Arabic speaking receptionist </h4>
                            </a>

                            <div class="mt-4">
                                <span class="cmp-teaser__job-listing-semibold" style="font-weight: 600;">Business
                                    Area : </span> Sales
                            </div>

                            <div class="cmp-teaser__job-posted mt-2">
                                <p class="cmp-teaser__job-listing cmp-teaser__job-listing-posted-date">
                                    Posted 1 day ago
                                </p>
                            </div>


                        </div>
                    </div>
                </div>

            </div>
        </div>
    </div>



    <!-- <div class="section-3 wf-section">
        <div>
            <div class="container ready_bg">
                <div class="row justify-content-between items-center">
                    <div class="col-md-7">
                        <h2 class="aside-title">
                            Join the InterCharge <span class="teal">Revolution</span>: Powering the future of Electric
                            Mobility
                        </h2>
                        <p class="aside-p">
                            Gain a competitive advantage by joining the world’s largest
                            eRoaming network.
                        </p>
                        <a href="<?php echo base_url('Contactus'); ?>" data-w-id="c17c272e-a5d2-ba46-c5b9-42170fb868df"
                            class="aside-btn button w-button">Get Started Today</a>
                    </div>
                </div>
            </div>
        </div> -->
    </div>
</section>